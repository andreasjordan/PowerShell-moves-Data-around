...
| The outbox is a real transaction, since 2026-08-16 | `photoservice-app.ps1` wraps the `UPDATE order_header` and the `INSERT INTO order_event` in one `BeginTransaction`, for payments and for shipments. They used to be two autocommitted statements, so demo 6 had to explain the gap rather than the pattern; the owner decided to close it instead. **Do not split them again** — the invariant is checked by `verify/06_eventstreaming.ps1`, in both directions and with a precondition that there were rows to compare. The order header and its details were already one transaction; what is deliberate there is that the Kafka events are produced *after* the commit, never inside it. |
