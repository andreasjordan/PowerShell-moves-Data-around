...
| **A full install has not been run since Kafka landed** | Everything committed on 2026-08-15 was verified against containers that were already up, by starting the new services into a running stack. `01_setup.ps1` has **not** been run start to finish since — so the ordering it depends on is reasoned and spot-checked rather than observed end to end: `03_pwsh_setup.ps1` fetching both platforms' drivers before `04_docker_compose.sh` starts the PhotoService container, which mounts `lib/` read only. The two halves of `03` were each run by hand and both exit 0. This is the next thing to do, from a fresh WSL2 with `lib/*.dll` and `lib/*.so` deleted. |
...
Where it is granted, two limits remain, and the second is a trap rather than a rule:
...
- **`docker compose down -v` is a twenty-minute mistake.** The `-v` deletes the volumes and getting
  them back means another Oracle start. There is almost never a reason — `docker compose stop` keeps
  the data and costs a minute.
